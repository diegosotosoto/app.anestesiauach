...
//variables
		...
		$titulo_navbar="<span class='fs-5'><img class='pe-2' src='images/icon.png' style='width: 48px' /></span>Login <small class='ps-5 opacity-50' style='font-size: 10px'>&nbsp;</small>";
  ...
  //si existe la cookie se salta el area de login y va al index
	...
	if(empty($_POST['email_usuario_v'])){
		...
		$sql="SELECT `password`,`nombre_usuario` FROM `usuarios_rf` WHERE `email_usuario`= '$email_usuario_v' AND `verified`= '1'";
		...
		if(mysqli_num_rows($result_sql)==0){
		      ...
		      ";
				...
				if($confirma_pass){
					...
					setcookie("hkjh41laa8u4l1k23jhlkj1387s76d8as76a9sd8",$galletita_mail, time()+60*60*24*30*6);
					setcookie("hkjh41laa8u4l1k23jhl54fgd897d1k23jhlkj14",$galletita_user, time()+60*60*24*30*6);
...
?>
	...
	if(empty($_POST['email_usuario'])){
		...
		$chequea_email="SELECT `email_usuario` FROM `usuarios_rf` WHERE `email_usuario`= '$email_usuario' AND `verified`= '1'";
		$result=$conexion->query($chequea_email);
		$conteo=mysqli_num_rows($result);

		if($conteo==0){
			
			$nuevo_usuario="INSERT INTO `usuarios_rf` (`nombre_usuario`, `email_usuario`, `password`) VALUES ('$nombre_usuario','$email_usuario','$pass_cifrado')";
